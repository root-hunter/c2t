...
C2T_EMBEDDED_USED const volatile unsigned char
    c2t_embedded_region[C2T_EMBEDDED_REGION_SIZE] = {
        ...
        2, 0, 0, 0 /* format version, followed by length and CRC32 */
    };
                                     ...
                                     size_t len)
{
    ...
    while (offset < len) {
        ...
        for (size_t i = 0; i < block_bytes; ++i)
            plaintext[offset + i] = (unsigned char)(ciphertext[offset + i] ^ keystream[i]);
...
static void ensure_decrypted_config(void)
{
    ...
    if (version == 2) {
        ...
        for (size_t i = 0; i < 12; ++i)
            nonce[i] = (unsigned char)c2t_embedded_region[C2T_EMBEDDED_HEADER_SIZE + i];
        size_t ciphertext_len = length - 12;
        chacha20_decrypt_payload(
            nonce,
            c2t_embedded_region + C2T_EMBEDDED_HEADER_SIZE + 12,
            decrypted_config,
            ciphertext_len);
        decrypted_config_len = ciphertext_len;
    } else if (version == 1) {
        for (size_t i = 0; i < length; ++i)
            decrypted_config[i] = (unsigned char)c2t_embedded_region[C2T_EMBEDDED_HEADER_SIZE + i];
        ...
        if (line_length > name_length &&
            ...
            if (index == name_length) {
                ...
                for (index = 0; index < value_length; ++index)
                    output[index] = (char)decrypted_config[line_start + name_length + 1 + index];
