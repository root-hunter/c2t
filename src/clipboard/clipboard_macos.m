...
static void copy_utf8(NSString *value, char *output, size_t capacity) {
  ...
  size_t copied = encoded_length < capacity - 1 ? encoded_length : capacity - 1;
...
static int capture_source(c2t_clipboard_source_t *source) {
  ...
  copy_utf8(application_name, source->application, sizeof(source->application));
  ...
  c2t_log_debug("macos",
                "Captured source application: app=%s, title=%s, "
                "pid=%lu",
...
static int output_image(NSPasteboard *pasteboard,
  ...
  static NSString *const direct_types[] = {
      @"public.png", @"public.jpeg", @"public.webp", @"com.compuserve.gif",
      @"com.microsoft.bmp"};
  static const char *const mime_types[] = {
      "image/png", "image/jpeg", "image/webp", "image/gif", "image/bmp"};
  NSArray<NSPasteboardType> *types = pasteboard.types;
  for (size_t index = 0; index < sizeof(direct_types) / sizeof(direct_types[0]);
       ++index) {
    if ([types containsObject:direct_types[index]] &&
        output_data(pasteboard, direct_types[index], mime_types[index], source))
      return 1;
  }
  ...
  NSData *png = image ? [image representationUsingType:NSBitmapImageFileTypePNG
                                            properties:@{}]
                      : nil;
