...
static size_t copy_property_text(const char *value, size_t length,
                                 char *output, size_t capacity)
{
    ...
    if (copied < length) {
        while (copied > 0 &&
               ((unsigned char)value[copied] & 0xc0) == 0x80)
            --copied;
    }
...
static int capture_window(xcb_connection_t *connection,
                          ...
                          xcb_atom_t name_atom, xcb_atom_t wm_name_atom,
                          ...
                          c2t_clipboard_source_t *source)
{
    ...
    int captured = active_window != XCB_WINDOW_NONE && capture_window(
        connection, active_window, name_atom, wm_name_atom, class_atom,
        pid_atom, source);
                         ...
                         xcb_atom_t selection, uint8_t *event_type)
{
    ...
    xfixes_query_version_request_t version_request = {
    ...
    sequence = xcb_send_request(
        connection, XCB_REQUEST_CHECKED, select_parts + 2, &select_protocol);
    error = xcb_request_check(
        connection, (xcb_void_cookie_t){ .sequence = sequence });
    if (error) {
        free(error);
        return 0;
    }
...
static int append_transfer(clipboard_transfer_t *transfer,
...
static int read_property(xcb_connection_t *connection, xcb_window_t window,
                         xcb_atom_t property, xcb_atom_t incr,
                         int incremental, clipboard_transfer_t *transfer,
                         ...
                         const c2t_clipboard_source_t *source)
{
    ...
    if (incremental) {
        if (length > 0 && !append_transfer(
                transfer, xcb_get_property_value(reply), (size_t)length)) {
            c2t_log_error("x11", "Clipboard content is too large");
            transfer->length = 0;
            incremental = 0;
        } else if (length == 0) {
            c2t_log_debug("x11",
                          "Incremental transfer complete (%llu bytes)",
                          (unsigned long long)transfer->length);
            clipboard_output(transfer->data, transfer->length, mime_type,
                             source);
            transfer->length = 0;
            incremental = 0;
        }
    } else {
        clipboard_output(xcb_get_property_value(reply), (size_t)length,
                         mime_type, source);
    }
...
static int clipboard_listen_once(void)
{
    ...
    if (c2t_config_get()->telegram_send_window_info &&
        ...
        c2t_log_warning(
            "x11", "Wayland session detected: source window information is "
    ...
    while (!c2t_runtime_stop_requested()) {
        ...
        if (event_type == xfixes_event_type) {
            ...
            if (selection->owner != XCB_WINDOW_NONE) {
                ...
                xcb_convert_selection(connection, window, clipboard, targets,
                                      property, selection->timestamp);
                xcb_flush(connection);
            }
            ...
            if (selection->property != XCB_ATOM_NONE &&
                requested_target == targets) {
                requested_target = select_target(
                    connection, window, property, supported,
                    sizeof(supported) / sizeof(supported[0]), &mime_type);
                if (requested_target != XCB_ATOM_NONE) {
                    c2t_log_debug("x11", "Selected clipboard target %s",
                                  mime_type);
                    xcb_convert_selection(connection, window, clipboard,
                                          requested_target, property,
                                          selection->time);
                    xcb_flush(connection);
                } else {
                    c2t_log_warning("x11",
                                    "Clipboard has no supported MIME type");
                }
